...
class CustomContainerRoyalesList extends StatefulWidget {
  @override
  _CustomContainerRoyalesListState createState() =>
      _CustomContainerRoyalesListState();
}

class _CustomContainerRoyalesListState
    extends State<CustomContainerRoyalesList> {
  ...
  void initState() {
   ...
   Future<List<Actualite>> _combineFutures(List<Future<List<Actualite>>> futures) {
    ...
    Future<List<Actualite>> fetchActualites(int _nombreActuCharges) async {
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=58&per_page=$_nombreActuCharges');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=3&per_page=$_nombreActuCharges');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=58&per_page=$_nombreActuCharges'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
...
// void main() {
//   runApp(MaterialApp(
//     home: CustomContainerRoyalesList(),
//   ));
// }
