...
class CustomContainerEconomieList extends StatefulWidget {
  @override
  _CustomContainerEconomieListState createState() =>
      _CustomContainerEconomieListState();
}

class _CustomContainerEconomieListState
    extends State<CustomContainerEconomieList> {
  ...
  Future<List<Actualite>> fetchActualites(int _nombreActuCharges) async {
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=26&per_page=$_nombreActuCharges');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=9&per_page=$_nombreActuCharges');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=26&per_page=$_nombreActuCharges'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
...
// Future<List<Actualite>> fetchActualites(int _nombreActuCharges) async {
//   final response = await http.get(Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=11&per_page=$_nombreActuCharges'));
