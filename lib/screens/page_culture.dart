...
class CustomContainerCultureMediaList extends StatefulWidget {
  @override
  _CustomContainerCultureMediaListState createState() =>
      _CustomContainerCultureMediaListState();
}

class _CustomContainerCultureMediaListState extends State<CustomContainerCultureMediaList> {
  ...
  _actualites = fetchActualites(_nombreActuCharges);
   ...
   Future<List<Actualite>> _combineFutures(List<Future<List<Actualite>>> futures) {
    ...
    Future<List<Actualite>> fetchActualites(int _nombreActuCharges) async {
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=28&per_page=$_nombreActuCharges');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=11&per_page=$_nombreActuCharges');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=28&per_page=$_nombreActuCharges'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
      ...
      body: FutureBuilder<List<Actualite>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            child: Text(
              lang =='fr' ? "Afficher plus de détails":'عرض المزيد من التفاصيل',
              style: TextStyle(color: Colors.white),
            ),
            style: ElevatedButton.styleFrom(
              backgroundColor: Color(0xFF1F72A5),
