...
class CustomContainerOpinions extends StatefulWidget {
  @override
  _CustomContainerOpinionsState createState() =>
      _CustomContainerOpinionsState();
}

class _CustomContainerOpinionsState extends State<CustomContainerOpinions> {
  ...
  Future<List<Actualite>> fetchActualites() async {
    lang = Provider.of<LangProvider>(context, listen: false).language!;
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=29&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=12&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=29&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
              ...
              child: Column(
                ...
                children: <Widget>[
                    Row(
                      ...
                      children: [
                        Text(
                          lang == 'fr' ? 'Opinions et Débats' : 'الآراء والمناقشات',
                          style: TextStyle(
                            fontSize: 20,
