...
class CustomContainerSocietesCiviles extends StatefulWidget {
  @override
  _CustomContainerSocietesCivilesState createState() =>
      _CustomContainerSocietesCivilesState();
}

class _CustomContainerSocietesCivilesState
    extends State<CustomContainerSocietesCiviles> {
  ...
  Future<List<Actualite>> fetchActualites() async {
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=25&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=8&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=25&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
          ...
          return InkWell(
            onTap: () {
               Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              CustomContainerSocietesCiviles(),
                        ),
                      );
            },
            child: Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                    Row(
                      ...
                      children: [
                        Text(
                          lang == 'fr' ? 'Société Civile et vie associative' : 'المجتمع المدني والحياة النقابية',
                          style: TextStyle(
                            fontSize: 20,
