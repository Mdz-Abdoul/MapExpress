...
import 'package:share_plus/share_plus.dart'; // Importez le package pour l'analyse HTML
...
class CustomContainerCultures extends StatefulWidget {
  @override
  _CustomContainerCulturesState createState() =>
      _CustomContainerCulturesState();
}

class _CustomContainerCulturesState extends State<CustomContainerCultures> {
  ...
  Future<List<Actualite>> fetchActualites() async {
    lang = Provider.of<LangProvider>(context, listen: false).language!;
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=28&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=11&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=28&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
          ...
          return InkWell(
            onTap: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      CustomContainerCultureMediaList(),
                ),
              );
            },
            child: Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                    Row(
                      ...
                      children: [
                        Text(
                          lang == 'fr' ? 'Culture et Média' : 'الثقافة والإعلام',
                          style: TextStyle(
                            fontSize: 20,
