...
class CustomContainerPrincieres extends StatefulWidget {
  @override
  _CustomContainerPrincieresState createState() =>
      _CustomContainerPrincieresState();
}

class _CustomContainerPrincieresState extends State<CustomContainerPrincieres> {
  ...
  Future<List<Actualite>> fetchActualites() async {
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=43&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=20&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=43&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
          ...
          return InkWell(
            onTap: () {
              Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => CustomContainerPrincieresList(),
                        ),
                      );
            },
            child: Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                    Row(
                      ...
                      children: [
                        Text(
                          lang == 'fr' ? 'Activités Princières' : 'إعدادات',
                          style: TextStyle(
                            fontSize: 20,
