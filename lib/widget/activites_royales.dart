...
class CustomContainers extends StatefulWidget {
  @override
  _CustomContainersState createState() => _CustomContainersState();
}
...
class _CustomContainersState extends State<CustomContainers> {
  ...
  Future<List<Actualite>> fetchActualites() async {
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=58&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=3&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=58&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
          ...
          return InkWell(
            onTap: () {
               Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => CustomContainerRoyalesList(),
                        ),
                      );
            },
            child: Container(
