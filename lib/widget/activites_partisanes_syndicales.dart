...
class CustomContainerPartisanes extends StatefulWidget {
  @override
  _CustomContainerPartisanesState createState() =>
      _CustomContainerPartisanesState();
}

class _CustomContainerPartisanesState extends State<CustomContainerPartisanes> {
  ...
  Future<List<Actualite>> fetchActualites() async {
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=23&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=7&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=23&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
          ...
          return InkWell(
            onTap: () {
               Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => CustomContainerPartisanesList(),
                        ),
                      );
            },
            child: Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                    Row(
                      ...
                      children: [
                        Text(
                          lang == 'fr' ? 'Activités Partisanes et Syndicales' : 'النشاط الحزبي والاتحادي',
                          style: TextStyle(
                            fontSize: 20,
