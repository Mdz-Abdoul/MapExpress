...
class CustomContainerParlements extends StatefulWidget {
  @override
  _CustomContainerParlementsState createState() =>
      _CustomContainerParlementsState();
}

class _CustomContainerParlementsState extends State<CustomContainerParlements> {
  ...
  Future<List<Actualite>> fetchActualites() async {
    lang = Provider.of<LangProvider>(context, listen: false).language!;
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=22&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=5&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=22&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
          ...
          return InkWell(
            onTap: () {
                Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => CustomContainerParlementsList(),
                        ),
                      );
            },
            child: Container(
