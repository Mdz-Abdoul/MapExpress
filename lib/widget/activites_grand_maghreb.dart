...
class CustomContainerGrandMaghreb extends StatefulWidget {
  @override
  _CustomContainerGrandMaghrebState createState() =>
      _CustomContainerGrandMaghrebState();
}

class _CustomContainerGrandMaghrebState
    extends State<CustomContainerGrandMaghreb> {
  ...
  Future<List<Actualite>> fetchActualites() async {
    lang = Provider.of<LangProvider>(context, listen: false).language!;
      ...
      if (lang == 'fr') {
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=54&per_page=1');
      } else if (lang == 'ar') {
        apiUri = Uri.parse('https://www.mapexpress.ma/ar/wp-json/wp/v2/actualite?actualites=25&per_page=1');
      } else {
        // Gérez le cas par défaut si lang n'est ni 'fr' ni 'ar'
        apiUri = Uri.parse('https://www.mapexpress.ma/wp-json/wp/v2/actualite?actualites=54&per_page=1'); // Remplacez 'URL_PAR_DEFAUT' par l'URL souhaitée en cas de défaut
      }
          ...
          return InkWell(
            onTap: () {
               Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      CustomContainerGrandMaghrebList(),
                ),
              );
            },
            child: Container(
